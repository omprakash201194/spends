...
### Feature — Bank Account Context ✅ COMPLETE
Bank name + account number shown under every transaction row; optional account filter on Dashboard and View detail pages. Consistent `>1 accounts` guard means single-account users see no extra UI.

- **TransactionPage** — bank badge (`bankName · accountNumberMasked`) rendered in `TxRow` merchant cell as `text-[10px] text-gray-400` below tags
- **ViewDto.TransactionItem** — `String bankName` added as 10th field; `ViewService.toTransactionItem` populates via null-safe `account != null ? account.getBankName() : ""`; `ViewTransactionItem` TypeScript interface updated; `ViewDetailPage` ListTab renders `{tx.bankName}` below raw remarks
- **Dashboard filter (backend)** — 7 new `*Filtered` JPQL methods in `TransactionRepository` (`sumWithdrawalsFiltered`, `sumDepositsFiltered`, `countInPeriodFiltered`, `categoryBreakdownFiltered`, `monthlyTrendFiltered`, `topMerchantsFiltered`, `latestTransactionDateFiltered`), all using `CAST(:accountId AS uuid) IS NULL OR t.bankAccount.id = CAST(:accountId AS uuid)`; `DashboardService.getSummary(userId, accountId)` routes all 12 aggregate calls through `*Filtered` variants; `DashboardController` gains `@RequestParam(required = false) UUID accountId`
- **Dashboard filter (frontend)** — `getDashboardSummary(accountId?: string)` in `api/dashboard.ts`; `DashboardPage` adds `accountId` state + `['bank-accounts']` query; selector renders only when `accounts.length > 1`; queryKey includes `accountId`
